...
def prepare_dirs_and_logger(config):
    ...
    if config.load_path:
        ...
        print(4)
        config.model_name = get_time() +'_'+ str(config.output_names) + '_layers_' + config.hidden + '_lr_' + str(config.lr)  + '_ac_' + str(config.act) + '_conv_' + str(config.convo) + '_locconv_' + str(config.localConvo) + '_vars_' + config.input_names + '_batchs_' + str(config.batch_size) + '_loss_' + str(config.lossfct) + config.addon#"{}_{}_{}".format(config.dataset, get_time(), ','.join(config.hidden.split(',')))
